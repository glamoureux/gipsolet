...
public class MapActivity extends FragmentActivity implements IGeolocationListener, OnMarkerClickListener, OnMapLongClickListener {
    ...
    private void drawCampus() {
    	if(map != null) {
    		if(refreshMap) {
				map.addPolygon(Campus.coordinates
		        		.fillColor(Color.argb(128, 128, 0, 0))
		        		.strokeColor(Color.argb(128, 255, 0, 0))
		        		.strokeWidth(5.0f));
		        map.addMarker(new MarkerOptions()
		        	.position(Campus.latlng)
		        	.title("Campus Triolet")
    ...
    private void drawMobile() {
    	if(map != null) {
    		if(mobile == null) {
	    		mobile = map.addMarker(new MarkerOptions()
    	...
    	if(map != null) {
			...
			map.animateCamera(CameraUpdateFactory.newLatLngBounds(builder.build(), 20), new CancelableCallback() {
				...
				public void onFinish() {
					...
					map.animateCamera(CameraUpdateFactory.newCameraPosition(new CameraPosition.Builder(map.getCameraPosition()).tilt(camera_tilt).build()));
				}
    ...
    private void drawMobileFocusTransition() {
    	if(map != null) {
    		if(mobile != null) {
    			...
    			map.animateCamera(CameraUpdateFactory.newLatLngZoom(mobile.getPosition(), 18.0f), new CancelableCallback() {
		    		...
		    		for(Building building: db.getBuildings()) {
		    			if(!building.getId().equals(researchBuildingId)) {
			    			Polygon polygon =  map.addPolygon(new PolygonOptions()
								.addAll(building.getShape())
								.fillColor(Color.argb(128, 0, 176, 240))
